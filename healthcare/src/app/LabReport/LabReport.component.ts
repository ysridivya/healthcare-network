...
export class LabReportComponent implements OnInit {
  ...
  byLab = new FormControl('', Validators.required);
  forDoctor = new FormControl('', Validators.required);

  constructor(public serviceLabReport: LabReportService, fb: FormBuilder) {
  ...
  loadAll(): Promise<any> {
    const tempList = [];
    return this.serviceLabReport.getAll()
    .toPromise()
    .then((result) => {
  ...
  addAsset(form: any): Promise<any> {
    this.asset = {
      $class: 'org1.healthcare.biznet.LabReport',
      'reportID': this.reportID.value,
      'ofPatient': this.ofPatient.value,
      'byLab': this.byLab.value,
    ...
    return this.serviceLabReport.addAsset(this.asset)
    .toPromise()
    .then(() => {
      this.errorMessage = null;
      this.myForm.setValue({
  ...
  updateAsset(form: any): Promise<any> {
    ...
    return this.serviceLabReport.updateAsset(form.get('reportID').value, this.asset)
    .toPromise()
    .then(() => {
      this.errorMessage = null;
      this.loadAll();
  ...
  deleteAsset(): Promise<any> {
    ...
    return this.serviceLabReport.deleteAsset(this.currentId)
    .toPromise()
    .then(() => {
      this.errorMessage = null;
      this.loadAll();
  ...
  getForm(id: any): Promise<any> {
    ...
    return this.serviceLabReport.getAsset(id)
    .toPromise()
    .then((result) => {
      this.errorMessage = null;
      const formObject = {
      ...
      if (result.byLab) {
        formObject.byLab = result.byLab;
      } else {
        formObject.byLab = null;
      }
