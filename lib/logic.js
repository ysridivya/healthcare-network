...
/**
 * Processes Prescription, adds entry that patient medicines were given
 * @param {org1.healthcare.biznet.ProcessLabReportRequest} preq
 * @transaction
 */

async function processLabReportRequest(preq){
    console.log("processLabReportRequest: Start");
    var patientid_condition = preq.labReportRequest.patientID != preq.patientID
    var stale_condition = preq.labReportRequest.authorized
    if(patientid_condition || stale_condition) {
      if (patientid_condition) {
        throw new Error('processPrescription Error: Patient Mismatch');
      }else {
        throw new Error('processPrescription Error: Lab report already authorized.');
      }
    }
    console.log("processPrescription: Constrains Satisfied!");
    let labReportRequestRegistry = await getAssetRegistry('org1.healthcare.biznet.RequestLabReport');
    preq.labReportRequest.authorized = true
    // Update the prescriptionRegistry in the asset registry.
    await labReportRequestRegistry.update(preq.labReportRequest);
    console.log("processLabReportRequest: Lab Report Request Approved!");
  }
...
async function processLabReport(generateLabReport){
    ...
    try{
      ...
      var labReportRequestRegistry =await getAssetRegistry('org1.healthcare.biznet.RequestLabReport');
      var find_labReportRequest = await labReportRequestRegistry.get(generateLabReport.labReportRequestID);

      if (!find_labReportRequest.authorized) {
        throw new Error('Patient has not authorized this request yet!');
      }
...
/**
 * Doctor request for lab report of patient
...
async function requestLabReport(request){
  ...
  var appointmentRegistry =await getAssetRegistry('org1.healthcare.biznet.Appointment');
  var find_appointment = await appointmentRegistry.get(request.appointmentID);
  console.log("requestLabReport: Got appointment: "+find_appointment.appointmentId);

  var doctor_condition = find_appointment.forDoctor.getIdentifier() != request.fromDoctor.getIdentifier()
  var patient_condition = find_appointment.byPatient.getIdentifier() != request.PatientId
  ...
  new_request.doctorID = request.fromDoctor.getIdentifier();
  new_request.patientID = request.PatientId
