 ...
 */
/*function IsAuthorized(asset, participant) {
    patient_Ns = "org1.healthcare.biznet.Patient";
    doctor_Ns = "org1.healthcare.biznet.Doctor";
    var found = false;
    if(participant.getFullyQualifiedType() == patient_Ns || participant.getFullyQualifiedType() == doctor_Ns)
    {
        let shareReportRegistry = getAssetRegistry('org1.healthcare.biznet.ShareReport');
        let shareReports = shareReportRegistry.getAll();
        shareReports.forEach(function(report){
            if(asset.reportId == report.reportId && report.sharedTo.getIdentifier() == participant.getIdentifier())
            {
                found = true;
            }
        });
