...
export class RestService {
  ...
  signUp(data) {
    const collector = {
      ...
      DoctorId: data.fullname,
      employedBy: 'resource:org1.healthcare.biznet.Hospital#'+data.hospitalid
    };
    return this.httpClient.post('http://localhost:3001/api/org1.healthcare.biznet.Doctor', collector).toPromise()
    .then(() => {
      const identity = {
        // TODO: Add logic to generate unique ids.
        participant: 'org1.healthcare.biznet.Doctor#' + data.fullname,
...
createAppointment(data) {
  console.log('createapoointment called by the user' + data.byDoctor)
  const collector = {
...
resolveAppointment(data) {
  console.log('resolveapoointment called by the user' + data.byDoctor)
  const collector = {
    $class : 'org1.healthcare.biznet.ResolveAppointment',
    appointment : 'resource:org1.healthcare.biznet.Appointment#'+data.appId,
    reportMessage : data.report,
    PrescriptionText : data.meds,
  ...
  return this.httpClient.post('http://localhost:3001/api//org1.healthcare.biznet.ResolveAppointment', collector).toPromise().then((opt) =>{
    console.log(opt);
